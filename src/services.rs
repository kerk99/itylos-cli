...
pub fn send_secret(api: &ItylosApi, options: SendOptions) -> Result<()> {
    ...
    println!("\n{}", "🦋 CAPSULE SECURISEE AVEC SUCCES".green());
    println!("{}", "─".repeat(50));
    println!("{} {}", "LIEN SECRET :".cyan().bold(), link);
    println!("{}", "─".repeat(50));
    println!(
        "{}",
        "La cle (#...) n'a jamais quitte cet ordinateur."
            .bright_black()
            .italic()
    );
    Ok(())
    ...
    if burn.success {
        if let Some(proof) = burn.proof {
            ...
            if signature_ok {
                println!(
                    "{}",
                    "🔥 Capsule detruite du serveur. Preuve de destruction verifiee.".green()
                );
            } else {
                println!(
                    "{}",
                    "⚠ Capsule detruite du serveur, mais la preuve Ed25519 n'a pas pu etre verifiee."
                        .yellow()
                );
            }
        } else {
            println!(
                "{}",
                "🔥 Capsule detruite du serveur. Preuve de destruction generee.".green()
            );
        }
    } else {
        println!(
            "{}",
            "⚠ Dechiffrement reussi, mais la purge serveur a retourne une erreur.".yellow()
        );
    }
    Ok(())
}
...
pub fn verify_proof(path: &Path) -> Result<()> {
    ...
    if crypto::verify_proof_signature(&canonical)? {
        println!(
            "{}",
            "✔ PREUVE AUTHENTIQUE : La destruction a ete confirmee cryptographiquement par ITYLOS."
                .green()
        );
    } else {
        println!(
            "{}",
            "✘ PREUVE FALSIFIEE : La signature ne correspond pas a l'empreinte de la donnee.".red()
        );
    }
    Ok(())
}
...
fn build_capsule(text: String, file: Option<PathBuf>) -> Result<CapsuleV3> {
    ...
    if let Some(file_path) = file {
        ...
        println!(
            "{}",
            format!("✓ Fichier charge : {} ({} octets)", file_name, file_size).cyan()
        );
    } else {
        if text.is_empty() {
